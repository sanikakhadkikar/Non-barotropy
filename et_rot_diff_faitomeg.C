...
namespace Lorene {
double et_rot_diff_fzero(double fomeg, const Param& par) ;
double uryu_subs(double fomeg) ; 
...
void Et_rot_diff::fait_omega_field(double f_omeg_min, double f_omeg_max,
    ...
    // Loop on the grid pointsq1	gvf	 t
    // -----------------------
	...
	for (k=0; k<mg.get_np(l); k++) {
		...
		for (i=0; i<mg.get_nr(l); i++) {
		    ...
		    double f_omeg_min1, f_omeg_max1 ; 
		    if ( prev_zero || fomeg == double(0)) {
			f_omeg_min1 = f_omeg_min ; 
			f_omeg_max1 = f_omeg_max ; 
		    } 
		    else{
			f_omeg_min1 = 0.8 * fomeg ; 
			f_omeg_max1 = 1.2 * fomeg ; 
		    } 
		
		    fomeg = zerosec(et_rot_diff_fzero, par, f_omeg_min1,
			 	   f_omeg_max1, precis, nitermax, niter) ;
			 	   
		    
		    omeg = uryu_subs(fomeg) ;
...
double et_rot_diff_fzero(double fomeg, const Param& par) {
	...
	const Cmp& brst2 = par.get_cmp(1) ;
   	...
   	double omeg = uryu_subs(fomeg) ;
   		
     	double omnp = omeg - nphi(l, k, j, i) ;
    	
     	return fomeg - brst2(l, k, j, i) * omnp /
...
double uryu_subs(double fomeg) {
	...
	float B = 1.0;
	float omeg_c = 3140.0;
	float denom = (1 + fomeg/(A*A*omeg_c))*(1 + fomeg/(A*A*omeg_c))*(1 + fomeg/(A*A*omeg_c))*(1 + fomeg/(A*A*omeg_c)) ;
	return omeg_c*(1 + (fomeg/(B*B*omeg_c)))/denom ;
